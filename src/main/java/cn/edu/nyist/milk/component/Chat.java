package cn.edu.nyist.milk.component;
...
public class Chat {
	 ...
	 * 连接建立成功调用的方法
	 * 
	 * @param session 可选的参数。session为与某个客户端的连接会话，需要通过它来给客户端发送数据 @PathParam("sendUser")
	 *                表示可以在访问的时候带上参数，参数：sendUser为 @ServerEndpoint("/chatDemo/{sendUser}")
	 * @throws IOException
	 */
	@OnOpen
	public void onOpen(@PathParam("sendUser") String sendUser, Session session) throws IOException {
		...
		if (StringUtils.isNotBlank(sendUser)) {
			Iterator<Entry<String, Chat>> iterator = webSocketMap.entrySet().iterator();
			while (iterator.hasNext()) {
				String key = iterator.next().getKey();
				if ("buyer".equals(key.split("_")[0])) {
					// 这里面只考虑商家使用一个用户名登录情况，最好的方法就是限制商家使用一个用户名登录
					webSocketMap.get(key).sendMessage("users", sendUser);
				}
			}
		}
	...
	public void sendMessage(String type, String message) throws IOException {
		if (type.equals("count")) {
			...
			logger.info("users为===============:" + message);
			this.session.getBasicRemote().sendText("users:" + message);
		} else {
