...
<body>
 ...
 <div id="shopcarthead">
					...
					<table class="table table-hover mx-auto" style="width: 92%;">
						...
						<tbody class="col-md-12">
							...
							<c:choose>
								...
								<td class="col-md-1 col-1 text-center" id="payStauts">${order.payStatus }</td>
								<td class="col-md-2 col-1 text-center">
								<a style="text-decoration: none;" href="front/orderitems?orderid=${order.orderId }">订单详情</a>
								<p><a style="text-decoration: none;" href="front/delorderitems?orderid=${order.orderId }">删除订单</a>
							 
							   <span class="spantopay">  
								    <a id="toPay" style="text-decoration: none;" href="javascript:void(0);" value="${order.payStatus }" text="${order.orderId }">去支付</a>
							   </span>
								</td>
							</tr>
	...
	$(function(){ 
 $('.spantopay a').click(function(){
 $(this).parent().each(function () {//移除其余非点中状态
	 var is=$(this).children("a").attr("value");//
	 var orderId=$(this).children("a").attr("text");
	 if(is==1){
		  alert("您已经支付过了！");
		  document.getElementById("toPay").href="javascript:void(0)";
	  }else{
		top.location.href="front/toEditPayStatus?orderid="+orderId;
	  }
 });
	 ...
	 /* function pay(){
		 ...
		 var doc=document.getElementById("payStauts"+${order.orderId }).innerText;
		 alert(doc);
		  if(doc==1){
			  alert("您已经支付过了！");
			  document.getElementById("toPay").href="javascript:void(0)";
		  }else{
			 // top.location.href="front/toEditPayStatus?orderid="+${order.orderId};
		  }
	 } */
	</script>
</body>
